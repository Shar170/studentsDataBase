...
class Test {
	private:
		...
		void Test5() {
			std::vector<Student> database;
			size_t initialSize = database.size();
			std::cout << "Запуск теста 5: Попытка добавления студента с отрицательным возрастом...\n";
			Student student4 = { "Иванова", 15, "История", -3.0 };
			database.push_back(student4);
			if (database.size() == initialSize) {
				std::cout << "Тест 5 успешно пройден. Студент с отрицательной успеваемостью не добавлен.\n\n";
			}
			else {
				std::cout << "Тест 5 не пройден. Ожидалось, что студент с отрицательной успеваемостью не будет добавлен.\n\n";
			}
		};
